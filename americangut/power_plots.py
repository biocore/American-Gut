...
def summarize_effect(order, fecal_cats, a_eff_means, a_eff_bounds, b_eff_means,
                     b_eff_bounds, correct=True):
...
def add_average_trace(fig, power, counts, labels, **kwargs):
    ...
    axes = fig.axes
    axes[0].plot(counts, site_lower, ':', color=kwds['bodysite_color'])
    axes[1].plot(counts, site_upper, ':', color=kwds['bodysite_color'])
    axes[2].plot(counts, site_mean, linewidth=2, color=kwds['bodysite_color'],
                 label=labels)

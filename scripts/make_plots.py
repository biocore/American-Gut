...
def interface():
    ...
    args.add_argument('-m', '--mapping-file', help='Mapping file',
                      required=True)
    args.add_argument('-t', '--taxa-file', help='Taxa summary file',
                      required=True)
    args.add_argument('-k', '--key-taxa-file', help='List of taxa to examine')
    args.add_argument('-o', '--output-prefix', help='Output file prefix',
                      default='./out')
    args.add_argument('-f', '--output-type', help='Output file type',
                      default='pdf')
...
def make_stacked_plot(output_file, filtered_sample_ids, taxa_labels,
    ...
    for i in xrange(1, N):
        ax1.fill_between(x, 1-cumulative[i-1, :], 1-cumulative[i, :],
                         color=colors[i])
...
def make_legend(output_file, taxa_labels, colors):
    ...
    seperate_legend.legend(wedges, taxa_labels, 'center', prop=font_prop,
                           frameon=False)
